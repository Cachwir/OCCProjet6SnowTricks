...
class ImageResizer {
     ...
     * Resize an image
     *
     * @param   string|UploadedFile   $image (The full image path with filename and extension)
     * @param   string                $newPath (The new path to where the image needs to be stored)
     * @param   string                $newFilename (The resized file's name. If null, keeps the original file's name)
     ...
     */
    public function resizeImage($image, $newPath, $newFilename = null, $height=0, $width=0){

        if ($image instanceof UploadedFile) {
            if ($newFilename === null) {
                throw new \Exception("The new filename needs to be specified if the image is an UploadedFile.");
            }
            $extension = $image->guessExtension();

            $image->move($newPath, $newFilename . "." . $extension);
            @chmod($newPath . $newFilename . "." . $extension, 0777);
            $image = $newPath . $newFilename . "." . $extension;
        }

        // Get current dimensions
        $ImageDetails = $this->getImageDetails($image);
        $name = $newFilename !== null ? $newFilename : $ImageDetails->name;
